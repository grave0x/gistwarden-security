...
1. **[`packages/domain/src/crypto.ts`](../../packages/domain/src/crypto.ts)**: Hàm tính
   toán mã TOTP HMAC-SHA1 (`generateTotpCode`, `parseTotpSecret`).
2. **[`packages/domain/src/generator-utils.ts`](../../packages/domain/src/generator-utils.ts)**:
   Bộ sinh Mật khẩu ngẫu nhiên (`generatePassword`), Cụm từ
   (`generatePassphrase`) và hàm sinh số ngẫu nhiên an toàn tuyệt đối
   (`getRandomBoundedInt`).
3. **[`packages/ui/src/features/generator/Generator.tsx`](../../packages/ui/src/features/generator/Generator.tsx)**:
